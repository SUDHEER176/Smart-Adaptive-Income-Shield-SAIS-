...
import { predictionService, insuranceService } from '../api';
      ...
      <div className="prediction-card main">
        ...
        <div className="prediction-details">
          <div className="stat">
            ...
            <value className="highlight">₹{prediction?.tomorrow?.predictedLoss || 0}</value>
          </div>
          <div className="stat">
            <label>🎯 Confidence</label>
            <value className="confidence-score">{prediction?.tomorrow?.confidence || 0}</value>
          </div>
          <div className="stat">
            <label>⚠️ Risk Level</label>
            <value className={`risk-${prediction?.tomorrow?.riskLevel?.toLowerCase() || 'low'}`}>
              {prediction?.tomorrow?.riskLevel || 'LOW'} 🔴
            </value>
          </div>
          <div className="stat">
            <label>🌦️ Weather</label>
            <value>{prediction?.tomorrow?.weather || 'Unknown'}</value>
          </div>
        </div>
